  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        ...
        const googleMapsURL = `https://maps.googleapis.com/maps/api/timezone/json?location=${lat},${lng}&timestamp=${Math.floor(Date.now() / 1000)}&key=${process.env.REACT_APP_GOOGLE_MAPS_KEY}`;
        // Get the current time at the zip code using timezonedb API
        axios
          .get(googleMapsURL)
          .then((response) => {
            console.log(response.data);
            const tZone = response.data.timeZoneId;


            console.log(tZone);
            let dateWithoutTZone = new Date(); // Get the current date and time
            console.log(dateWithoutTZone);
            let currentDate = new Date(dateWithoutTZone.toLocaleString("en-US", {
              timeZone: tZone,
            ...
            axios
              .get(visible_planets_url)
              .then((response) => {
                setVisibleObjects(response.data.data);
